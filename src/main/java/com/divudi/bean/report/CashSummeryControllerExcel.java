...
@Named
@RequestScoped
public class CashSummeryControllerExcel implements Serializable {
    ...
    public List<BillsItems> getCreditCompanyCollection() {
        ...
        for (Bill b : tmp) {
            ...
            temMap.put("toDate", getToDate());
            temMap.put("ins", getInstitution());
            newB.setBillItems(getBillItemFacade().findBySQL(sql, temMap, TemporalType.TIMESTAMP));
...
//    }
    ...
    public List<DepartmentPayment> getDepartmentPayment() {
        ...
        String sql = "SELECT b FROM BillItem b WHERE b.bill.institution=:ins and b.retired=false and b.bill.billType= :btp and b.createdAt between :fromDate and :toDate";
    ...
    public List<BillsItems> getDoctorPayment() {
        ...
        for (Bill b : tmp) {
            ...
            temMap.put("toDate", getToDate());
            temMap.put("ins", getInstitution());
            nB.setBillItems(getBillItemFacade().findBySQL(sql, temMap, TemporalType.TIMESTAMP));
    ...
    public List<Bill> getRecieveBill() {
        ...
        h.put("ins", getInstitution());

        return getBillFacade().findBySQL(sql, h, TemporalType.DATE);
    }
...
//    }
    ...
    private List<Category> findCategory() {
        ...
        temMap.put("fromDate", getFromDate());
        temMap.put("ins", getInstitution());
        temMap.put("ins2", getInstitution());
        temMap.put("bTp", BillType.OpdBill);
        temMap.put("pm1", PaymentMethod.Cash);
        temMap.put("pm2", PaymentMethod.Card);
        temMap.put("pm3", PaymentMethod.Cheque);
...
//    }
    ...
    public void createOPdCategoryTable() {
        ...
        for (CategoryWithItem dc : getDailyCash2()) {
            ...
            for (ItemWithFee iwf : dc.getItemWithFees()) {
                String1Value2 newD = new String1Value2();
                newD.setString(iwf.getItem().getName());
